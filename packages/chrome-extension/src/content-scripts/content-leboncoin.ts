...
console.log('[Annoncify] LeBonCoin content script loaded')

const adapter = platformRegistry.getAdapter('leboncoin')

if (!adapter) {
  console.error('[Annoncify] LeBonCoin adapter not found')
}

browser.runtime.onMessage.addListener((message: unknown): Promise<ChromeMessageResponse> => {
  return (async () => {
    // Vérifie que message est bien un objet ChromeMessage
    ...
    try {
      switch (msg.action) {
        ...
        case 'GET_ACTIVE_PLATFORM':
          return { success: true, data: { platform: 'leboncoin' }, requestId: msg.requestId } as ChromeMessageResponse
...
// Message de détection de la plateforme
...
console.log("Content script Leboncoin loaded")
export {}